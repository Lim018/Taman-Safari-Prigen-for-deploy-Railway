...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto bg-white rounded-lg shadow-md overflow-hidden">
        <div class="bg-[#274E13] text-white py-4 px-6">
            <h2 class="text-2xl font-bold">Beri Testimoni Taman Safari</h2>
        </div>
        <form action="{{ route('testimonials.store') }}" method="POST" class="p-6">
            @csrf
            <div class="mb-4">
                <label for="MESSAGE_TEXT" class="block text-gray-700 text-sm font-bold mb-2">Pesan Anda:</label>
                <br>
                <textarea 
                    name="MESSAGE_TEXT" 
                    id="MESSAGE_TEXT" 
                    rows="5" 
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('MESSAGE_TEXT') border-red-500 @enderror"
                    placeholder="Bagikan pengalaman Anda di Taman Safari..."
            ...
            <div class="flex items-center justify-between">
                <button type="submit" class="text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline transition duration-300" style="color: #274E13;">
                    Kirim Testimoni
                </button>
            </div>
        </form>
    ...
    @if (session('success'))
        <div class="mt-4 max-w-2xl mx-auto bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative" role="alert">
