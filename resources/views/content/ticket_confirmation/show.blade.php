    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    <h2>{{ $ticket->content->TITLE }}</h2>
                    <p><strong>Ticket Number:</strong> {{ $ticket->ticket_number }}</p>
                    <p><strong>Type:</strong> {{ ucfirst($ticket->ticket_type) }}</p>
                    <p><strong>Quantity:</strong> {{ $ticket->quantity }}</p>
                    <p><strong>Booking Date:</strong> {{ $ticket->booking_date->format('d F Y') }}</p>
                    <p><strong>Status:</strong> {{ ucfirst($ticket->status) }}</p>

                    @if($ticket->status === 'unused')
                        <form action="{{ route('ticket.confirm.post', $ticket->ticket_number) }}" method="POST">
                            @csrf
                            <button type="submit" class="btn btn-primary">Confirm and Use Ticket</button>
                        </form>
                    @else
